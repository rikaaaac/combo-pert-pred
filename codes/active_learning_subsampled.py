"""
Active Learning Loop for Combinatorial Perturbation Prediction - SUBSAMPLED VERSION

This version subsamples the data to ~20% to speed up computation.
...
# ================================================================================
# Main - SUBSAMPLED VERSION
# ================================================================================

if __name__ == "__main__":
    from ensemble_full_pipeline import Ensemble
    ...
    RESULT_DIR = '/insomnia001/depts/edu/BIOLBC3141_Fall2025/rc3517/ml-stats/active_learning_results_subsampled'
    ...
    # ==========================================
    print("=" * 60)
    print("SUBSAMPLING DATA (20% of cells)")
    print("=" * 60)

    subsampled_path = f'{DATA_DIR}/norman_perturbseq_subsampled.h5ad'

    # Check if subsampled file already exists
    if os.path.exists(subsampled_path):
        print(f"Subsampled file already exists: {subsampled_path}")
        print("Using existing subsampled data.\n")
    else:
        print("Creating subsampled dataset...")
        adata_full = sc.read_h5ad(f'{DATA_DIR}/norman_perturbseq_preprocessed_hvg_filtered.h5ad')
        print(f"Original data: {adata_full.n_obs} cells, {adata_full.n_vars} genes")

        # Subsample to 20% of cells using numpy (avoiding scanpy compatibility issue)
        np.random.seed(42)
        n_cells = adata_full.n_obs
        n_subsample = int(n_cells * 0.2)
        subsample_indices = np.random.choice(n_cells, size=n_subsample, replace=False)
        adata_full = adata_full[subsample_indices, :].copy()
        print(f"Subsampled data: {adata_full.n_obs} cells, {adata_full.n_vars} genes")

        # Save subsampled data
        adata_full.write_h5ad(subsampled_path)
        print(f"Saved to: {subsampled_path}\n")
        del adata_full

    # ==========================================
    # LOAD ENSEMBLE WITH SUBSAMPLED DATA
    # ==========================================
    print("Loading ensemble with subsampled data...")
    ...
    ensemble.load_models(
        ...
        sclambda_adata_path=subsampled_path,  # Use subsampled data
        sclambda_embeddings_path=f'{DATA_DIR}/GPT_3_5_gene_embeddings_3-large.pickle',
        norman_data_path=subsampled_path  # Use subsampled data
    )
    ...
    # ==========================================
    # RUN ACTIVE LEARNING WITH REDUCED ITERATIONS
    # ==========================================
    print("\n" + "=" * 60)
    print("STARTING ACTIVE LEARNING (10 iterations, 5 samples per iteration)")
    print("=" * 60)
    ...
    # ==========================================
    # SHAP ANALYSIS WITH REDUCED PARAMETERS
    # ==========================================
    print("\n" + "=" * 60)
    print("RUNNING SHAP ANALYSIS (reduced parameters)")
    print("=" * 60)

    al = ActiveLearningLoop(ensemble)
    al.run_shap_analysis(
        ensemble.X_combo,
        n_background=50,  # Reduced from 100
        n_explain=25      # Reduced from 50
    )
    al.plot_shap_results(RESULT_DIR)

    print(f"\n{'=' * 60}")
    print(f"DONE! All results saved to {RESULT_DIR}")
    print(f"{'=' * 60}")
